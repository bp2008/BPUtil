...
namespace BPUtil
{
	...
	{
		/// <summary>
		/// RFC 4180 declares that the CSV line ending is CRLF (\r\n).
		/// </summary>
		...
		/// </summary>
		/// <param name="str">The string to encode as a CSV field</param>
		/// <returns>Possibly-quoted output string safe to insert into a CSV file as a cell value.</returns>
		...
		/// <summary>
		/// <para>Quote every CSV field whether it needs it or not, to help make it idiot-proof.</para>
		/// <para>Input <c>test</c> becomes <c>"test"</c>.</para>
		/// <para>Input <c>Substring "with" quotes</c> becomes <c>"Substring ""with"" quotes"</c>.</para>
		/// </summary>
		...
		/// <param name="str">The string to validate.</param>
		/// <exception cref="ArgumentException">Thrown if the string contains any invalid characters.</exception>
		public static void ThrowIfStringIsInvalidUnicodeCSVValue(string str)
		{
			if (str != null)
			{
				foreach (char c in str)
				{
					if ((c > (char)31 && c < (char)127) || c == '\r' || c == '\n' || c > (char)127) { }
						...
						throw new ArgumentException("Value contained (char)" + ((int)c) + " which is not allowed in Unicode CSV (comma-separated values) files.");
				}
			}
		}
		/// <summary>
		/// Returns a copy of the given string with any invalid characters replaced with spaces.  Allowed character values are 10 (\n), 13 (\r), and 32-126.
		/// </summary>
		/// <param name="str">The string to process.</param>
		...
		/// <summary>
		/// Returns a copy of the given string with any characters not allowed in a unicode CSV file replaced with spaces.  Allowed character values are 10 (\n), 13 (\r), 32-126, and 128 and higher.
		/// </summary>
		/// <param name="str">The string to process.</param>
		/// <returns>The string with invalid characters replaced.</returns>
		public static string ReplaceInvalidUnicodeCharactersWithSpaces(string str)
		{
			if (str == null)
				return "";
			StringBuilder sb = new StringBuilder();
		...
		/// <summary>
		/// Returns a string that contains this table in CSV (comma-separated values) format.  This method returns a string compatible with US-ASCII, containing only character values 10 (\n), 13 (\r), and 32-126.
		/// </summary>
		...
		/// <param name="replaceOutOfRangeCharactersWithSpaces">If true, all out-of-range characters will be replaced with spaces.  If false, an exception will be thrown if an out-of-range character is encountered while creating the CSV string.  Notably, TAB is an out-of-range character.</param>
		...
		/// <summary>
		/// Returns a string that contains this table in CSV (comma-separated values) format.  This method returns a string compatible with UTF-8, containing character values 10 (\n), 13 (\r), 32-126, and 128 and higher.
		/// </summary>
		/// <param name="table">This DataTable.</param>
		/// <param name="includeHeaders">If true, the first row of the file will contain the column names.</param>
		/// <param name="replaceOutOfRangeCharactersWithSpaces">If true, all out-of-range characters will be replaced with spaces.  If false, an exception will be thrown if an out-of-range character is encountered while creating the CSV string.  Notably, TAB is an out-of-range character.</param>
		/// <returns>A string that contains this table in CSV (comma-separated values) format.</returns>
		/// <exception cref="ArgumentException">If any of the values in the table contain a character that is out of range or reserved in CSV files, and <paramref name="replaceOutOfRangeCharactersWithSpaces"/> is false.</exception>
		public static string ToCommaSeparatedValuesUnicode(this DataTable table, bool includeHeaders = false, bool replaceOutOfRangeCharactersWithSpaces = false)
		{
			StringBuilder result = new StringBuilder();
			...
			if (includeHeaders)
			{
				for (int i = 0; i < table.Columns.Count; i++)
				{
					string str = table.Columns[i].ColumnName;
					if (replaceOutOfRangeCharactersWithSpaces)
						str = ReplaceInvalidUnicodeCharactersWithSpaces(str);
					else
						ThrowIfStringIsInvalidUnicodeCSVValue(str);
		...
		/// <returns></returns>
		public static string RemoveTrailingEmptyCsvValues(string input)
		{
			if (input == null)
				return null;
			...
			{
				foreach (string line_loopVaribale in CSVFile.StreamingSplitIntoLines(sr))
				{
					string line = line_loopVaribale;
					string before = null;
					while (line != before)
					{
						before = line;
		...
		/// <summary>
		/// Replaces all line ending sequences with CRLF as per CSV spec.
		/// </summary>
		/// <param name="input"></param>
		/// <returns></returns>
		public static string NormalizeLineBreaks(string input)
		{
			...
			using (StringReader sr = new StringReader(input))
			{
				IEnumerable<string> lines = CSVFile.StreamingSplitIntoLines(sr);
				return string.Join(LineEnding, lines);
			}
		}
	}
	...
	public class CSVFile
	{
		...
		/// <summary>
		/// <para>Reads CSV rows from the stream and allows them to be processed row by row without needing to load the entire stream into memory at once.</para>
		/// <para>When each row is read, it is returned as a single string via yield return.</para>
		/// <para>If rowCallback returns true, the next row is read.</para>
		/// <para>If rowCallback returns false, the function returns early.</para>
		/// <para>Exceptions may be thrown if the CSV file is determined to be invalid.</para>
		/// </summary>
		/// <param name="tr">A <c>TextReader</c> (<c>StreamReader</c> or <c>StringReader</c>) positioned at the beginning of a line in a csv file.</param>
		public static IEnumerable<string> StreamingSplitIntoLines(TextReader tr)
		{
			string line;
			while ((line = ReadLogicalLine(tr)) != null)
			{
		...
		/// <summary>
		/// Reads a logical CSV line from the TextReader.  This may read more than one physical line if there are embedded line breaks within quoted fields.  For example, the following two lines in a CSV file is one logical line:
		/// <code>
		/// StartOfLine,"Quoted
		/// Value",EndOfLine</code>
		/// </summary>
		/// <param name="tr">TextReader</param>
		/// <returns>A logical CSV line, or null if the end of the TextReader was reached.</returns>
		public static string ReadLogicalLine(TextReader tr)
		{
